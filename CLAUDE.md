...
| `ui/camera/grid/brazing_client.{h,cpp}` | GUI-thread `QObject`: async best-effort POST of the combined snapshot to `{base_url}/api/brazing/update` via `QNetworkAccessManager` (5 s timeout, log-and-drop, no retry). |
