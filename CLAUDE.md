...
| `denso_brazing` (`brazing/`) | reporting logic: `brazing_payload`, `brazing_retry_policy`, `zone_aggregator` (+ contracts `zone_reading.h`, `zone_sink.h`, `brazing_transport.h`) |
| `denso_camera` (`camera/`) | non-widget capture/frame infra: `frame_convert`, `gst_pipeline`, `rtsp_templates`, `snapshot`, `fps_meter`, `stream_pacing`, `warmup_gate`, `zone_assembly`, `frame_processor` |
...
| Path | Responsibility |
|---|---|
| `main.cpp` | Thin orchestrator: install the logging handler → open DB → migrate → import legacy → sync models → load settings → apply startup → run. Network **reassert** is deferred to the first event-loop tick (`QTimer::singleShot(0, …)`, best-effort/exceptions swallowed) so a slow/stuck CLI can't keep the window from appearing. |
...
| `brazing/brazing_client.{h,cpp}` | `BrazingTransport` impl (stays in `denso`): one async POST of the snapshot to `{base_url}/api/brazing/update` (`QNetworkAccessManager`, 5 s timeout); reports 2xx-or-not via a `done(ok)` callback. No retry state (that's the reporter's). |
| `ui/camera/dialog/` (pages) | The dialog's five page widgets + shared `page_util` (`dim_label`): `list_page` (`CameraListPage`), `add_page` (Source form + scans; tags already-owned devices **"(in use)"** via the used-source set the controller pushes), `configure_page` (preview + orientation controls), `models_page` (`ModelsPage`: attach models + per-class conf, with **Select all / Clear** class-selection buttons), `areas_page` (ROI editing; shows the review-required prompt under quarantine). Pages own their controls and emit request signals; the `CameraWizardController` drives them. |
